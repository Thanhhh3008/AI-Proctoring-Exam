...
export class ProctoringEngine {
  ...
  private staticImageWarningCounter = 0; // Đếm 5s sau khi badge hiện
  private isStaticImage = false; // State bền vững giữa các lần chạy
  ...
  start(videoElement: HTMLVideoElement) {
    ...
    // Vòng lặp chính: face-api.js + MediaPipe + Optical Flow (mỗi 1s)
    this.analysisLoop = window.setInterval(() => this.runMainAnalysis(), 1000);
  ...
  private async runMainAnalysis() {
    ...
    if (this.faceVerification.isReady()) {
      try {
        const result = await this.faceVerification.verify(this.videoElement);
        ...
        if (faceDetected) {
          if (!result.isMatch) {
            ...
            // Chờ 10 lần detect (khoảng 10s) mới báo vi phạm người khác
            if (this.identityViolationCounter >= 10) {
              this.emitViolation('DIFFERENT_PERSON', { distance: result.distance });
              identityVerified = false;
            } else {
              // Trong lúc đang đếm, báo là chưa verify được
              identityVerified = false;
            }
          } else {
        ...
        if (faceCount > 1) {
          // Đối với nhiều người, cũng có thể chờ nếu muốn, nhưng thường là báo ngay
          // Ở đây ta cứ báo ngay hoặc chờ 3s
    ...
    // 3. Face Absence Detection (Chờ 10s)
    if (!faceDetected) {
      this.noFaceCounter++;
    ...
    // 4. Looking Away (Head Pose) (Chờ 10s)
    ...
    } else { this.lookingAwayCounter = 0; }
